            ...
            # Function calls
                ...
                if func_name == 'sum':
                    ...
                    if not all(isinstance(x, (int, float)) for x in values):
                        raise TypeError(
                            f"sum() only accepts numbers or lists of numbers\n"
                            f"on line {line} in {self.file}"
                        )
                    return sum(values)
                ...
                if func_name == 'product':
                    ...
                    if not values or not all(isinstance(x, (int, float)) for x in values):
                        raise TypeError(
                            f"product() expects numeric arguments or a list of numbers\n"
                            f"on line {line} in {self.file}"
                        )
                ...
                if func_name == 'quotient':
                    ...
                    if len(values) < 2:
                        raise TypeError(
                            f"quotient() needs at least two numbers\n"
                            f"on line {line} in {self.file}"
                        )
                    if not all(isinstance(x, (int, float)) for x in values):
                        raise TypeError(
                            f"quotient() only accepts numeric values\n"
                            f"on line {line} in {self.file}"
                        )
                    ...
                    for v in values[1:]:
                        if v == 0:
                            raise ZeroDivisionError(
                                f"division by zero on line {line} in {self.file}"
                            )
                        result //= v
                    return result
                ...
                if func_name == 'subtract':
                    ...
                    if len(values) < 2:
                        raise TypeError(
                            f"subtract() needs at least two numbers\n"
                            f"on line {line} in {self.file}"
                        )
                    if not all(isinstance(x, (int, float)) for x in values):
                        raise TypeError(
                            f"subtract() only accepts numeric values\n"
                            f"on line {line} in {self.file}"
                        )
